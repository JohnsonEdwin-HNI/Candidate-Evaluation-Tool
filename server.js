...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (['.pdf', '.doc', '.docx'].includes(ext)) {
      ...
      cb(new Error(`"${file.originalname}" is not a supported file type. Please upload a PDF or Word document.`));
    }
  }
});
    ...
    sections.forEach((paras, sIdx) => {
      children.push(new Paragraph({
        children: [new TextRun({
          ...
          bold: true, size: 22, color: 'C8440A', font: 'Arial', allCaps: true
        })],
